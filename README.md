...
| 類別 | 它在算什麼 |
|---|---|
| **重複讀取** | 同一 session 內，同一個檔（完整路徑）沒改過卻又讀了一次。中間有 Edit/Write 就重新計；Bash 指令不算，跑兩次本來就會不一樣 |
...
前面三條都是「**少放東西進主 context**」。這一條不一樣——它讓那些東西
**根本不進主 context**。

子代理在自己的視窗裡讀檔。它讀的東西只在它活著的那幾輪被計費，
結束就消失，只有結論回到主執行緒。

工具會把這件事量化：

```
主執行緒     4,238,793,150 tok   $2,963.72   91.0%   10822 turns
子代理         418,836,832 tok     $255.14    9.0%    1951 turns
...
### 但它跟「重開 session」是同一條法則

子代理要重新建立自己的開場 context，而那要付**寫入價**。所以：

```
划算的 parent session：5 個（平均 420 turns）
不划算的：            6 個（平均 136 turns）
```

**parent session 還很長才划算，快結束時開子代理反而更貴。**
...
- **事件大小是近似的。** 以字元數估：CJK 字元每個算 1 token，其他文字 4 字元／token。
  這樣估是因為 session 裡中文跟程式碼混在一起，用單一除數會把中文吃掉的 token 算到程式碼頭上。
  只有事件之間的**比例**有意義，單一事件被分到的那一份仍是近似值。
